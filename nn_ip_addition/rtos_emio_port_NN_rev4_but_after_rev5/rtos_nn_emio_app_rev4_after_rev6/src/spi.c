/*
 * spi.c
 *
 *  Created on: Jul 7, 2018
 *      Author: Jerome Pare-Lepine
 *      Notes: WIP - Needed for the LoRa telemetry implementation. The part is
 *             an RFM95 breakout from Adafruit. This driver would also be useful
 *             for reading SSI data without bitbanging.
 */

//==============================================================================
// INCLUDES
//==============================================================================
#include "spi.h"

void InitSpiInst(XSpiPs* inst)
/*
* \brief       Initializes the SPI controller struct instance using the autogenerated
*              bsp config.
*
* \param[in]   inst         - Pointer to the SPI peripheral struct instance to be
*                             initialized.
*/
{
	XSpiPs_Config *config;

	config = XSpiPs_LookupConfig(XPAR_PS7_SPI_0_DEVICE_ID);
	XSpiPs_CfgInitialize(inst, config, config->BaseAddress);

}

void SpiWrite(XSpiPs* inst, u32* data)
/*
 * WIP
 */
{

}

u32 SpiRead(XSpiPs* inst)
/*
 * WIP
 */
{

}


